...
node.default[:environment_variables]["EC2_INSTANCE_ID"] = `/opt/aws/bin/ec2-metadata -i`[/ (?<a>.*)n?$/, "a"]
node.default[:environment_variables]["EC2_AVAIL_ZONE"] = `/opt/aws/bin/ec2-metadata -z`[/ (?<a>.*)n?$/, "a"]
node.default[:environment_variables]["EC2_PUBLIC_IP4"] = `/opt/aws/bin/ec2-metadata -z`[/ (?<a>.*)n?$/, "a"]
node.default[:environment_variables]["HOST_NAME"] = `/opt/aws/bin/ec2-metadata -p`[/ (?<a>.*)n?$/, "a"]
